...
const extractReviewsHeading = (html) => {
  ...
  const reviewsHeadingMatch = html.match(
    /<h2[^>]*>\s*What our customers are saying[^<]*<\/h2>/i,
  );
...
const convertHomeContent = async () => {
  ...
  const reviewsHeading =
    extractReviewsHeading(html) ||
    getReviewsHeading("What our customers are saying...");
  ...
  const categoryPattern =
    /<div class="featured-categories__category-panel[^"]*"[^>]*>[\s\S]*?<a[^>]*href="([^"]+)"[^>]*>[\s\S]*?<img[^>]*src="([^"]+)"[^>]*>[\s\S]*?<\/a>[\s\S]*?<div class="featured-categories__title">([^<]+)<\/div>/g;
  let cardMatch;
  let cardIndex = 0;
  ...
  const headingMatch = html.match(
    /<div class="text-center margin-bottom-40">\s*<h1[^>]*>([\s\S]*?)<\/h1>/,
  );
  if (headingMatch) {
  ...
  const bodyContentMatch = html.match(
    /<div id="BodyContent"[^>]*>([\s\S]*?)<\/div>\s*<script>/,
  );
  if (bodyContentMatch) {
  ...
  const lowerContentMatch = html.match(
    /<div id="ctl00_HomeLowerContentPanel"[^>]*>([\s\S]*?)<\/div>\s*<div id="ctl00_PhotoGallery"/,
  );
  ...
  const productsPattern =
    /<div class="castlePanel">[\s\S]*?<img[^>]*alt="([^"]*)"[^>]*data-public-image="([^"]+)"[^>]*>[\s\S]*?<a[^>]*href="([^"]+)"[^>]*class="castleLink">([^<]+)<\/a>/g;
  let productMatch;
  let productIndex = 0;
  while (
    (productMatch = productsPattern.exec(html)) !== null &&
    productIndex < 8
  ) {
  ...
  const photoGalleryPattern =
    /<div class="photo-gallery__slide">[\s\S]*?<img[^>]*data-public-image="([^"]+)"[^>]*>/g;
  let logoMatch;
  let logoIndex = 0;
  while (
    (logoMatch = photoGalleryPattern.exec(html)) !== null &&
    logoIndex < 20
  ) {
  ...
  const areasMatch = html.match(
    /Areas we cover near you![\s\S]*?<\/p>([\s\S]*?)<p[^>]*>.*?You will find/i,
  );
  if (areasMatch) {
  ...
  console.log(
    `   - Ticker message: ${homeContent.banner.special_offer.message ? "Yes" : "No"}`,
  );
  console.log(`   - ${homeContent.hero.service_cards.length} service cards`);
  console.log(
    `   - ${homeContent.main_content.paragraphs.length} content paragraphs`,
  );
